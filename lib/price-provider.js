...
export async function lookupVerifiedPricing({
  ...
  let timedOut = false;
  const timeout = Math.max(1, Number(timeoutMs) || 7000);
  try {
    raw = await Promise.race([
      selectedProvider.lookup({
    ...
    return {
      status: timedOut || error?.code === 'PRICE_PROVIDER_TIMEOUT' ? 'PROVIDER_TIMEOUT' : 'PROVIDER_ERROR',
      providerId,
      sourceLabel: null,
      checkedAt: null,
      marketRange: null,
