...
function validateScenarioEvidence(entry, errors, warnings, { official = false } = {}) {
  ...
  if (id === 5) {
    ...
    const reviewedRedactedVin = official &&
      evidence?.vehicle?.vin == null &&
      evidence?.vehicle?.vinRedacted === true &&
      /^[a-f0-9]{64}$/.test(String(evidence?.vehicle?.vinSha256 || '')) &&
      evidence?.acceptance?.hasVin === true;
    if (!validVin(evidence?.vehicle?.vin) && !reviewedRedactedVin) {
...
export function validateFieldTestScenarioEvidence(entry = {}) {
  const errors = [];
  const warnings = [];
  const id = Number(entry?.id ?? entry?.scenarioId);
  const status = String(entry?.status || '').toUpperCase();

  if (!Number.isInteger(id) || !REQUIRED_IDS.includes(id)) {
    errors.push(`Invalid scenario id: ${entry?.id ?? entry?.scenarioId}`);
  }
  if (!VALID_STATUSES.has(status)) {
    errors.push(`Scenario ${Number.isInteger(id) ? id : '?'}: invalid status ${status || '(empty)'}`);
  }
...
export function validateOfficialFieldTestResults(document = {}) {
  ...
  for (const entry of scenarios) {
    ...
    validateScenarioEvidence({ ...entry, id, status }, errors, warnings, { official: true });
  }
