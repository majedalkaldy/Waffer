import { validateOfficialFieldTestResults } from './field-test-evidence-validator.js';
...
export function evaluateFieldTestResults(results = {}) {
  ...
  const evidenceValidation = validateOfficialFieldTestResults({ scenarios: normalized });
  const evidenceErrors = evidenceValidation.errors || [];
  const integrityErrors = [...new Set([...schemaErrors, ...evidenceErrors])];
  const statusBlockers = normalized
    .filter(item => item.status !== 'PASS')
    .map(item => 'Scenario ' + item.id + ' is ' + item.status);
  const allPassed =
    integrityErrors.length === 0 &&
    passed === REQUIRED_SCENARIO_IDS.length;

  return {
    schemaValid: schemaErrors.length === 0,
    evidenceValid: evidenceValidation.valid,
    integrityValid: integrityErrors.length === 0,
    ...
    statusBlockers,
    blockers: [...integrityErrors, ...statusBlockers],
...
export function evaluateLaunchGate({ launchPhase, results } = {}) {
  ...
  return {
    ...
    blockers: [
      ...
      ...fieldTest.integrityErrors,
      ...(phase === 'field-test' ? [] : fieldTest.statusBlockers)
    ]
  };
}
