...
export function summarizeFieldTestSession({
  official = {},
  automation = {},
  draft = {}
} = {}) {
  const dashboard = normalizeFieldTestDashboard({ official, automation, draft });
  const failed = dashboard.scenarios.filter(item => item.draftStatus === 'FAIL').map(item => item.id);
  const pending = dashboard.scenarios.filter(item => item.draftStatus === 'PENDING').map(item => item.id);
  const passed = dashboard.scenarios.filter(item => item.draftStatus === 'PASS').map(item => item.id);
  const nextScenarioId = failed[0] ?? pending[0] ?? null;
  ...
  return {
    ...
    failed,
    pending,
    completed: passed.length + failed.length,
    unresolved: failed.length + pending.length,
    allPassed: passed.length === FIELD_TEST_SCENARIO_IDS.length
  };
}
