...
function visiblePartNumber(value) {
  const text = String(value || '').trim();
  return Boolean(text) && !/غير ظاهر|غير متوفر|not visible|not available|unknown/i.test(text);
}
...
export function normalizeAnalysisResult({
  ...
  const normalizedItems = rawItems
    ...
    ? rawItems
        ...
        .map(item => {
          ...
          const fallbackType = visiblePartNumber(partNumber) ? 'part' : 'service';
          return {
            ...item,
  ...
  const hasVin = /^[A-HJ-NPR-Z0-9]{17}$/.test(String(safeVehicle?.vin || '').toUpperCase());
  const itemCount = normalized.items.length;
  const identifiedCount = normalized.items.filter(item => visiblePartNumber(item.partNumber)).length;
